## 0. 运行环境自检（每次对话第一动作，MUST执行）

**触发条件**：用户的第一条消息

**执行内容**：
1. 调用 `list_allowed_directories` 检测可用文件系统
2. 内部记录：
   - 可访问目录（如 `E:\`、`D:\`）
   - 是否有 Filesystem MCP（read/write/edit）
   - 是否有 `web_search`、`image_search` 等工具
3. 不输出给用户（除非用户问起）

**关键认知**：
- 如果 `list_allowed_directories` 返回 `E:\sanxiangwendao` 所在盘符 → 你就是操作员，直接读写，不需要Cursor
- 如果没有返回 → 才走"Claude生成+Cursor执行"的协作模式

---

## 0.1 工作模式优先级（覆盖原有"Claude-Cursor协作模式"章节）

**优先级排序**（从高到低）：

1. **【优选】直接执行模式**：
   - 触发：MCP Filesystem 可用 + 任务涉及本地文件
   - 行为：直接调用 `read_text_file` / `create_directory` / `write_file` 等工具
   - 不要说"我生成指令给Cursor"，直接执行

2. **【次选】Claude-Cursor协作模式**：
   - 触发：MCP不可用 或 用户明确要求"给Cursor指令"
   - 行为：生成精简指令，用户复制给Cursor
...
> **术语澄清**：
> - "Project Knowledge"指Claude.ai项目知识库（Web版）
> - "本地知识库"指 `E:\sanxiangwendao`（Claude Desktop+MCP）
> - 当用户在Claude Desktop使用，且 `list_allowed_directories` 返回 `E:\` → 一律按"本地知识库"处理，不要尝试 `project_knowledge_search`
...
Step 3.5: 案例搜索验证（新增，必须执行）——在写开篇案例前，用 web_search 搜索真实案例，列出2-3个候选供用户选择，用户确认后再进入 Step 4
Step 4: 内容生成（遵循内容写作7步法，逐段生成，每段800-1500字，每段完成后询问确认）
Step 5: YAML索引生成（按照框架指南模板生成完整RAG索引）
Step 6: 全局质检清单（11项必检，立场/案例/深度/完整性/案例真实性）
...
**Step 3.5：案例搜索验证（新增，必须执行）**

在写任何开篇案例之前，**必须先执行**：
```
🔍 案例搜索验证（禁止未经验证使用虚构人物作为核心案例）

1. 用 web_search 搜索与文档主题相关的真实案例（书籍、研究、公开报道）
2. 列出 2-3 个候选真实案例，附来源与一句话概括，供用户选择
3. 用户确认选用哪一个（或提供松阳本人亲身经历）后，才开始 Step 4 写作

禁止：在未经搜索验证前，使用虚构人物（如"李明"、"张三"）作为核心开篇案例。
例外：松阳本人的亲身经历（有第一手素材）可直接使用，无需搜索。
```
...
严格按照《新文档写作框架指南》中的"六、写作内容（7步法）"执行：

1. 故事/案例开头（200-500字，必须；案例须为 Step 3.5 已确认的真实案例或松阳亲身经历）
2. 核心洞察/问题（必须）
3. 理论框架/方法（必须）
4. 案例分析/对比（至少3个案例，必须）
5. 实践建议/行动（立即+长期，必须）
...
□ 7. 有没有"反常识"的冲击？
     ...
     注意：反常识不是硬性数量要求。一篇文章有1个被充分论证的反常识，强过3个只有结论没有机制的反常识。质检通过标准是"有且论证充分"，而不是"数量达标"。
...
- ✅ 理论文档写作：7步法完成？YAML索引生成？11项质量检查通过？
- ✅ **【新增】如果这次输出涉及全局调整（立场、框架、深度），标题是否需要同步更新？**

**❓ 问题4：我用对工具了吗？**
- 如果任务涉及本地文件，我是直接读写了，还是绕路给Cursor指令？
- 如果用户在Claude Desktop，我有没有先调 `list_allowed_directories` 自检？
- 如果绕路了，是我懒还是真的需要Cursor？
